...
// Oturumu başlat
session_start();
// Firma admin için kupon yönetimi
...
// 3. Form İşlemleri (Ekleme veya Silme)
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == 'add') {
        ...
        if (empty($code) || $discount_rate <= 0 || $discount_rate > 1 || $usage_limit <= 0 || empty($expiration_date)) {
            ...
            try {
                ...
                // Sadece kendi firmasına (company_id) ekle
                $stmt->execute([$coupon_id, $code, $discount_rate, $usage_limit, $expiration_date, $company_id]);
                $mesaj = "Kupon ('$code') başarıyla eklendi.";
    ...
    if ($action == 'delete') {
        ...
        if (empty($coupon_id)) {
            ...
            // Güvenlik: Sadece kendi firmasının kuponunu sil
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $hata_mesaji = "Kupon bulunamadı veya silme yetkiniz yok.";
            }
        }
    }
}

// 4. Bu firmaya ait TÜM kuponları çek
$stmt_coupons = $pdo->prepare("SELECT * FROM Coupons WHERE company_id = ? ORDER BY expiration_date ASC");
...
<body>
    ...
    <header>
        ...
        <nav>
            ...
            <a href="firma_admin_panel.php" class="nav-button-white">Sefer Yönetimine Dön</a>
    ...
    <main>
        ...
        <div class="form-container" style="border: 1px solid #ddd; padding: 15px; margin-bottom: 20px;">
            <h3>Yeni Kupon Oluştur</h3>
            <form action="kupon_yonetimi.php" method="POST">
        ...
        <table border="1" style="width: 100%; border-collapse: collapse;">
            ...
            <tbody>
                <?php if (empty($kuponlar)): ?>
                    <tr>
                        <td colspan="5" style="text-align: center;">Firmanıza ait kayıtlı kupon bulunmamaktadır.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($kuponlar as $kupon): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                |
                                <form action="kupon_yonetimi.php" method="POST" style="display:inline;" onsubmit="return confirm('Bu kuponu silmek istediğinizden emin misiniz?');">
